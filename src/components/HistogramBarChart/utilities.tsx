...
export const getColor = (colorArray: string[], index: number) => colorArray[index] || colorArray[colorArray.length - 1]

// Generates aria label for a bar
export const getAriaLabel = (bar: HistogramBar) => `${bar.value} - Exactly: ${bar.percentage.toFixed(0)}%, At Least: ${bar.atLeastPercentage.toFixed(0)}%`

/**
 * Generates an array of numbers to use for the gridYValues or gridXValues property of a Nivo chart
 */
export const getGridValues = (tickSize: number, min: number, max: number) => Array(Math.ceil((max - min + 1) / tickSize)).fill(0).map((_, index) => index * tickSize);
 ...
 */
export const renderDamageTicks = (tick: { y: number, x: number, value: number }) => (
...
type HistogramTooltipProps = { bar: HistogramBar }
 ...
 *     At Least: 32%
 */
export function HistogramTooltip({ bar }: HistogramTooltipProps) {
    return (
        <div className="bg-light p-2 rounded text-dark">
            {bar.value} <PropertyIcon property={DAM}/><br/>
            Exactly: {bar.percentage.toFixed(0)}%<br />
            At Least: {bar.atLeastPercentage.toFixed(0)}%
        </div>
    )
}